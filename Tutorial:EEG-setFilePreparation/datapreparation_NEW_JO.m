...
topPath = '/Applications/EEG/Steps/';
DataDir = [topPath, 'Data/'];
...
addpath(genpath('/Applications/EEG/eeglab2024.1/'))

% modify the participant and visit session for analysis
Pat = 'TD44'; Visit = 'Mon5';
ResultsDir = [topPath, 'Results/', Pat, '/', Visit, '/'];
mkdir(ResultsDir);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
datafolder = strcat(DataDir,Pat,'/',Visit,'/');
...
SessionDir = dir(strcat(datafolder,'*.txt'));
...
%% process files one by one - this is fetching the data 
...
for i = 1:length(SessionDir)
    ...
    extractEEG_biosemi_new(SessionDir, i, topPath, ResultsDir)
...
function extractEEG_biosemi_new(SessionDir, trial_idx, parentPathForCedFile, ...
    FolderToSaveResults)
...
if true % JO. This is not necessary
    ...
    EEG = pop_chanedit(EEG, 'lookup', [parentPathForCedFile, 'Processing/BioSemi_32Ch.ced'], 'load', ...
        {[parentPathForCedFile, 'Processing/BioSemi_32Ch.ced'], 'filetype', 'autodetect'});
    EEG = eeg_checkset(EEG);
    ...
    EEG = pop_saveset( EEG, 'filename', filename, 'filepath', FolderToSaveResults);
